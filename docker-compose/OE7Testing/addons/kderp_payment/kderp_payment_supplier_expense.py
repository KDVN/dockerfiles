...
class kderp_supplier_payment_expense_pay(osv.osv):
    _name = "kderp.supplier.payment.expense.pay"
    _description = "KDERP Supplier Payment Expense Pay"
    _rec_name='supplier_payment_expense_id'

    #SYSTEM METHOD    
    def create(self, cr, uid, vals, context={}):
        new_payment_id = super(kderp_supplier_payment_expense_pay, self).create(cr, uid, vals, context=context)
        ...
        for kp in self.browse(cr, uid, new_payment_ids):
            if kp.supplier_payment_expense_id.state<>'completed':
                list_failed.append(kp.supplier_payment_expense_id.name)
            else:
                kspe_ids.append(kp.supplier_payment_expense_id.id)
        ...
        for kspe_id in kspe_ids:
            kspe_obj.write(cr, uid, [kspe_id],{'state':'paid'})
...
#                 unlink_ids.append(kp['id'])
    ...
    _columns={
                ...
                'period_id': fields.many2one('account.period', 'Period',  states={'draft':[('readonly',False)]}),
                'currency_id': fields.many2one('res.currency', 'Currency',  required=True, states={'draft':[('readonly',False)]}),
                'amount': fields.float("Amount", digits_compute=dp.get_precision('Account'), required=True,  states={'draft':[('readonly',False)]}),
#                'amount_currency':fields.float("Amount Currency",digits_compute=dp.get_precision('Amount'), required=True,  states={'draft':[('readonly',False)]}),
                'exrate': fields.float("Ex.Rate",digits_compute=dp.get_precision('Amount'), required=True,  states={'draft':[('readonly',False)]}),
                'supplier_payment_expense_id':fields.many2one('kderp.supplier.payment.expense','Supplier Payment',required=True,domain="[('state','=','completed')]",   ondelete='restrict'),
                'move_id': fields.many2one('account.move', 'Detail',  select=1,ondelete='restrict'),
                'writeoff':fields.boolean('Write-Off',states={'draft':[('readonly',False)]}),
                'state':fields.selection([('draft','Draft'),('done','Done')],'State',readonly=True),
                'bank_id':fields.many2one('res.bank','Bank',states={'draft':[('readonly',False)]}),
              }
    _defaults = {
    ...
    _sql_constraints = [
                        ('supplier_payment_expense_pay_unique',"unique(supplier_payment_expense_id)","KDERP Error: The Supplier Payment must be unique !")
                        ]
...
#             if kp.supplier_payment_expense_id.move_id:
#                 for move_line in kp.supplier_payment_expense_id.move_id.line_id:
#                     move_line_ids.append(move_line.id)
            ...
            kspe_obj.write(cr, uid, [kp.supplier_payment_expense_id.id], {'state':'completed'}, context)
            
        return True
    
    def update_ksp_payment(self, cr, uid, ids, context):
        cr.execute("""Select kspp.id,ksp.name from kderp_supplier_payment_pay kspp left join kderp_supplier_payment ksp on supplier_payment_id=ksp.id where ksp.state='paid'""")
        kp_ids=[]
        
        ErrorList=[]
        for kp_id,name in cr.fetchall():
            try:
                self.action_reconcile(cr, uid, [kp_id],context)
                cr.commit()
            except ValueError, e:
                cr.rollback()
                new_error={
                            kp_id:{'Payment Number':name,'Error':unicode(e)}
                           }                
        return {}
    ...
    def action_reconcile(self, cr, uid, ids, context={}):
        ...
        for kp in self.browse(cr, uid, ids, context):
            if kp.currency_id.id<>company_currency.id:
                ...
                if not kp.exrate:
                    exrate = kp.supplier_payment_expense_id.expense_id.exrate
                else:
                    exrate = kp.exrate
                #Put Period later will write to Payment
                kp_vals['exrate']=exrate                        
            else:
                exrate=kp.amount/kp.supplier_payment_expense_id.total if kp.supplier_payment_expense_id.total else 0
                kp_vals['exrate']=1
            #Check write off or not

            check_writeoff = kp.writeoff             
            acc_writeoff = False
            if exrate<>kp.supplier_payment_expense_id.exrate:
            ...
            if context.get('new_exrate',0) and not check_writeoff:
                kspe_obj.action_cancel(cr, uid, [kp.supplier_payment_expense_id.id], context)
                kspe_obj.action_revising_completed(cr, uid, [kp.supplier_payment_expense_id.id], context)
            ...
            #Post
            #mark_as_paid = kspe_obj.write(cr, uid, [kp.supplier_payment_expense_id.id], {'state':'done'})
            cr.execute("""Update kderp_supplier_payment_expense set state='done' where id = %s """ % kp.supplier_payment_expense_id.id)
            ctx=context.copy()
            ctx['invoice'] = kp.supplier_payment_expense_id
    ...
    def pay_and_reconcile(self, cr, uid, ids, pay_amount, pay_account_id, period_id, pay_journal_id, writeoff_acc_id, writeoff_period_id, writeoff_journal_id, context=None, name=''):
        ...
        kspe = self.browse(cr, uid, ids[0], context=context)
        company = self.pool.get('res.users').browse(cr, uid, uid).company_id
        src_account_id = kspe.supplier_payment_expense_id.account_id.id
        ...
        ref = kspe.supplier_payment_expense_id.name
        
        partner = self.pool['res.partner']._find_accounting_partner(kspe.supplier_payment_expense_id.expense_id.partner_id)
        ...
        if not name:
            name = kspe.supplier_payment_expense_id.name
        l1['name'] = name
        l2['name'] = name
        ...
        move_ids = [move_id,]
        if kspe.supplier_payment_expense_id.move_id:
            move_ids.append(kspe.supplier_payment_expense_id.move_id.id)
        
        cr.execute('SELECT id FROM account_move_line '\
                   'WHERE move_id IN %s',
                   ((move_id, kspe.supplier_payment_expense_id.move_id.id),))
        lines = line.browse(cr, uid, map(lambda x: x[0], cr.fetchall()))
        for l in lines:#+ kspe.supplier_payment_expense_id.payment_ids
            if l.account_id.id == src_account_id:
                line_ids.append(l.id)
                total += (l.debit or 0.0) - (l.credit or 0.0)

        inv_id, name = self.name_get(cr, uid, [kspe.id], context=context)[0]
    ...
    #def import_data(self, cr, uid, fields, datas, mode='init', current_module='', noupdate=False, context=None, filename=None):
        try:
            payment_id_pos = fields.index('supplier_payment_expense_id')
        except:
            payment_id_pos = -1
...
#             raise osv.except_osv("KDVN Error",new_data)
            cr.execute("Select name from kderp_supplier_payment_expense krop where name in %s and state!='completed'" % (payment_nos))
            if cr.rowcount>0:
                 list1 =[]
                ...
                if list_to_get:
                    ...
                    cr.execute("Select krop.name,(%s) from kderp_supplier_payment_expense krop left join res_currency rc on currency_id=rc.id where krop.name in %s and state='completed'" % (list_field_str,payment_nos))
        ...
        return super(kderp_supplier_payment_expense_pay, self).load( cr, uid, list_field_to_import, data, context)
    
kderp_supplier_payment_expense_pay()
...
class kderp_supplier_payment_expense(osv.osv):
    ...
    _columns={
                'payment_ids':fields.one2many('kderp.supplier.payment.expense.pay','supplier_payment_expense_id','Payment', required=True,readonly=True, states={'completed':[('readonly',False)],'revising':[('readonly',False)]})
             }
